...
public sealed class VorticeMediaFoundationD3D11Decoder : IHighResolutionD3DDecoder
{
	...
	private readonly D3D11.ID3D11Device _device;
	...
	public event Action<IHighResolutionD3DFrame>? FrameDecoded;

	public event Action<string>? Faulted;

	public bool DumpH264Enabled { get; set; }

	public VorticeMediaFoundationD3D11Decoder(int width, int height, int fps, D3D11.ID3D11Device device)
	{
		_width = width;
		_height = height;
		_fps = Math.Max(1, fps);
	...
	private void DrainOutput(H264InputPacket packet)
	{
		...
		for (int i = 0; i < 64; i++)
		{
			...
			try
			{
				if (producedSample != null && TryCreateD3D11Frame(producedSample, packet, out VorticeD3D11VideoFrame? frame) && frame != null)
				{
					VorticeD3D11VideoFrame decodedFrame = frame;
	...
	private bool TryCreateD3D11Frame(IMFSample sample, H264InputPacket packet, out VorticeD3D11VideoFrame? frame)
	{
		frame = null;
		...
		try
		{
			...
			var texture = new D3D11.ID3D11Texture2D(texturePtr);
			D3D11.Texture2DDescription desc = texture.Description;
			int actualWidth = checked((int)desc.Width);
			int actualHeight = checked((int)desc.Height);
			int arraySize = checked((int)desc.ArraySize);
			if (desc.Format != Vortice.DXGI.Format.NV12 || !IsCompatibleNv12OutputSize(_width, _height, actualWidth, actualHeight))
			{
				string message =
					$"High-resolution D3D output geometry changed: expected {_width}x{_height} NV12, " +
					$"got {actualWidth}x{actualHeight} {desc.Format} from decoder texture.";
				texture.Dispose();
				texturePtr = IntPtr.Zero;
				Fault(message);
				throw new InvalidOperationException(message);
			}
			LogPaddedOutputGeometryIfNeeded(actualWidth, actualHeight, "decoder texture");
			if (subresourceIndex < 0 || subresourceIndex >= arraySize)
			{
				if (!_loggedInvalidTextureSubresource)
				{
					_loggedInvalidTextureSubresource = true;
					StatusChanged?.Invoke(
						$"Media Foundation D3D11 decoder rejected NV12 texture subresourceIndex={subresourceIndex}, arraySize={arraySize}.");
				}
				texture.Dispose();
				texturePtr = IntPtr.Zero;
				return false;
			...
			if (!_loggedFirstTextureDetails)
			{
				...
				StatusChanged?.Invoke(
					$"Media Foundation D3D11 first texture: format={desc.Format} size={actualWidth}x{actualHeight} " +
					$"arraySize={arraySize} subresourceIndex={subresourceIndex}.");
			}

			frame = new VorticeD3D11VideoFrame
			{
				Width = _width,
				Height = _height,
				Fps = _fps,
